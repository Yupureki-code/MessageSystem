...
namespace messageSystem
{
    ...
    public:
        ...
        std::string generateUniqueCode()
        {
            ...
            // 2. 随机数引擎（通常用 Mersenne Twister）
            std::mt19937 gen(rd());
            // 3. 定义分布范围 [100000, 999999]（闭区间）
        ...
        MailSendResult SendAuthCodeMail(const std::string& to, const std::string& code)
        {
            ...
            if (res != CURLE_OK)
            {
                ...
                LOG_ERROR("Send mail failed by libcurl, code={},smtp_reply={},detail={}",static_cast<int>(res),smtp_response_code,result.error);
                if (!dbg.server_lines.empty())
                {
                    LOG_ERROR("SMTP transcript:{}",dbg.server_lines);
                }
            }
            else
            {
